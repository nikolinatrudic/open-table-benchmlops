...
def create_spark_session(app_name: str, spark_cluster_ip: str):
    spark = SparkSession.builder.master(f"spark://{spark_cluster_ip}:7077").appName(app_name).getOrCreate()
    return spark
...
@click.command()
@click.option('--benchmark', type=click.Choice(['time-travel', 'ingestion'], case_sensitive=False), help='The benchmark to run.')
def main(benchmark):
